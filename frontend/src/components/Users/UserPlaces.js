...
const UserPlaces = () => {
  ...
  useEffect(() => {
    const fetchUserAndPlaces = async () => {
      try {
        // Fetch user details
        const userResponse = await axios.get(`http://localhost:5005/api/users/id/${userId}`);
        setUser(userResponse.data);

        // Fetch user's places
        const placesResponse = await axios.get(`http://localhost:5005/api/places/${userId}/places`);
